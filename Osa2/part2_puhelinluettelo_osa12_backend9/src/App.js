...
class App extends React.Component {
  ...
  numberUpdate = (id,personObject) => {
    const haveToKnow = '  on jo luettelossa, korvaatanko vanha numero uudella ? Please, press peruuta, the functionality still does not work!'
    if (window.confirm(personObject.name + haveToKnow)) {
    personService
    ...
    .catch(error => {
      this.setMessage(`${personObject.name} on jo valitettavasti poistettu palvelimelta`)
      this.setState({ 
  ...
  setMessage = (message) => {
    ...
    if (this.state.messageClass === "okMessage") {
    setTimeout(() => {
        this.setState({message:''})}, 1000)
    } else {
      setTimeout(() => {
        this.setState({message:"", messageClass:"okMessage"})}, 5000)
    }
  }
